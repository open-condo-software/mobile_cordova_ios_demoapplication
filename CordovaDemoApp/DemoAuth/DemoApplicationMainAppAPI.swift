...
@objc public class DemoApplicationMainAppAPI: NSObject {
    ...
    let demotoken = "zDDoQYfgFgvNJrBZ0kPnGTtDXaRPi9vc./4JPN1xJLWlYoC2RX4yWWb5ikavzMM9/3b5fP5yxqdE"
    ...
    @objc public func getCode(clientID: String, complition: @escaping (String?) -> Void) {
        let url = "https://v1.doma.ai/oidc/auth?response_type=code&client_id=\(clientID)&redirect_uri=\(self.redirectURI)&scope=openid&state=\(state)"
        client.performRequest(url: url, cookie: authorizationCookie()) { data in
